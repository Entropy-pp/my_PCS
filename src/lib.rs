...
/// 生成 u，长度为 s = 2 * t，每个元素都是随机的字段元素
pub fn generate_u<F: Field>(t: usize) -> Vec<F> {
    let s = 2 * t;
    let mut rng = test_rng();
    (0..s).map(|_| F::rand(&mut rng)).collect()
}
...
pub fn partial_sum<F: Field>(
    ...
    for k1 in &poly.polynomial().coeffs {
        h_coff[i / b] += *k1 * eq(i % b, u1, t);
        i += 1;
    }
    let h_poly = DensePolynomial::from_coefficients_vec(h_coff.clone());
...
pub fn folded_g<F: Field>(
    ...
    let mut g_coff: Vec<F> = Vec::new(); // 存储 g(x) 的系数
    let n = poly.degree() + 1;
    let b = n.isqrt();
    let mut f_coff: Vec<F> = Vec::new(); // 存储 f(x) 的系数
    for i in 0..b {
        for k in 0..b {
            f_coff.push(poly.coeffs[i + k * b].clone());
        }
        let f_poly = DensePolynomial::from_coefficients_vec(f_coff.clone());
        let sum = f_poly.evaluate(&alpha);
        g_coff.push(sum);
        f_coff.clear();
    }
    let g_poly = DensePolynomial::from_coefficients_vec(g_coff);
    LabeledPolynomial::new("g(x)".to_string(), g_poly, None, Some(2))
}
...
pub fn compute_q<F: Field>(
    ...
    let n = f.degree() + 1;
    let b = n.isqrt();
    let mut q_coff: Vec<F> = vec![F::zero(); n];
    let mut f_coff: Vec<F> = Vec::new(); // 用于存储分块的 f(x) 系数
    for i in 0..b{
        for k in 0..b{
            f_coff.push(f.coeffs[i + k * b].clone());
        }
        let f_poly = DensePolynomial::from_coefficients_vec(f_coff.clone());
        let qi_poly = divide_polynomial(&f_poly, alpha);
        let mut flag:usize = 0;
        for k in &qi_poly.coeffs{
            q_coff[i + flag * b] = *k;
            flag += 1;
        }
        f_coff.clear();
    }
    let q_poly = DensePolynomial::from_coefficients_vec(q_coff);
...
pub fn compute_d<F: Field>(
    ...
    let d_poly = DensePolynomial::from_coefficients_vec(new_coeffs);
...
pub fn pu_evaluate<F: PrimeField>(u: &[F], x: F) -> F {
    u.iter().enumerate().map(|(i, &ui)| {
        ...
        ui * x_pow + (F::one() - ui) // u_i * X^(2^i) + (1 - u_i)
    }).product() // 计算乘积
}
...
pub fn compute_s<F: PrimeField>(
    gr: &LabeledPolynomial<F, DensePolynomial<F>>,
    hr: &LabeledPolynomial<F, DensePolynomial<F>>,
    u1: &[F],
    u2: &[F],
    gamma: F,
    b: usize,
...
pub fn compute_big_h<F: PrimeField>(
    ...
    let numerator = f - term2 - DensePolynomial::from_coefficients_vec(vec![g_z]);

    // 除以 X - z：多项式除法
    let denominator = DensePolynomial::from_coefficients_vec(vec![-z, F::one()]); // X - z

    let h_poly = numerator / denominator; // 使用多项式除法
    LabeledPolynomial::new("H(x)".to_string(), h_poly, None, Some(2))
}
...
pub fn compute_batch_f_w_l_w_hat<F: PrimeField>(
    ...
    DensePolynomial<F>, // batch_f
    LabeledPolynomial<F, DensePolynomial<F>>, // w
    DensePolynomial<F>, // batch_l
    LabeledPolynomial<F, DensePolynomial<F>>, // w_hat
){
    let mut poly_1 = DensePolynomial::from_coefficients_vec(vec![F::zero()]);
    let mut poly_2 = DensePolynomial::from_coefficients_vec(vec![F::zero()]);
    for i in 0..fr.len() {
        let gamma_pow_i = gamma.pow([i as u64]);
        let s_i = &s[i];
        let t_s = difference(&t, s_i);
        let z_s_i = compute_zs(&t_s);
        let z_s_i_2 = z_s_i.clone();

        // -------- poly_1 ------------
        let f_r = fr[i] - rr[i];
        let scaled_1 = f_r.mul(gamma_pow_i);
        poly_1 = &poly_1 + &z_s_i.mul(scaled_1);

        // -------- poly_2 ------------
        let eval_r = rr[i].evaluate(&z);
        let f_r_at_z = fr[i] - DensePolynomial::from_coefficients_vec(vec![eval_r]);
        let scaled_2 = f_r_at_z.mul(gamma_pow_i);
        let eval_z_s_i = z_s_i_2.evaluate(&z);
        poly_2 = &poly_2 + &(scaled_2 * eval_z_s_i);
    }
    let w = compute_batch_w(poly_1.clone(), t.clone());

    let z_t = compute_zs(&t);
    let eval_z_t = z_t.evaluate(&z);
    let correction = w.polynomial().mul(eval_z_t);

    let l = &poly_2 - &correction;

    let w_hat = compute_batch_w_hat(l.clone(), z);
...
pub fn compute_batch_w_hat<F: Field>(
    ...
    let w_poly = l / poly;
    LabeledPolynomial::new("w(x)".to_string(), w_poly, None, Some(2))
}
